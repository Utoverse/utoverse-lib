package cn.utoverse.utoverselib.economy;

import cn.utoverse.utoverselib.UtoverseLibPlugin;
import cn.utoverse.utoverselib.economy.layers.VaultLayer;
    ...
    public static void onEnable() {
        Schedulers.sync().run(() -> {
            ...
            for (final Plugin plugin : Bukkit.getPluginManager().getPlugins()) {
                ...
                if (layer != null) {
                    UtoverseLibPlugin.getInstance().getLogger().log(Level.INFO, "Essentials found a compatible payment resolution method: " + layer.getName() + " (v" + layer.getPluginVersion() + ")!");
                }
            }
            onServerLoad();
        });
